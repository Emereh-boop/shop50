import React from "react";
import { Facebook, Instagram, TwitterX, Whatsapp } from "react-bootstrap-icons";

export default function Footer() {
  return (
    <footer
      className={`px-10 static gap-5 flex flex-col bg-primary dark:invert-0 invert text-white`}
    >
      <div className="flex text-7xl font-black justify-center md:text-9xl">
        YNT
      </div>
      <hr className="text-gray-400" />
      <div className="flex gap-4 md:gap-2 justify-between flex-col md:flex-row">
        <div className="lg:col-span-2">
          <h1 className="uppercase font-semibold mb-4">NEWSLETTER</h1>
          <p className="mb-4">
            Stay in touch with the latest updates about our products and offers.
          </p>
          <form method="post" href="#" className="flex items-center">
            <input
              type="email"
              className="flex-1 p-2 rounded-l-sm bg-transparent text-primary ring-1 ring-secondary focus:outline-none"
              placeholder="Email"
            />
            <button
              type="submit"
              className="p-2 bg-secondary rounded-r-md text-primary hover:bg-secondary/80"
            >
              Join
            </button>
          </form>
        </div>
        <div className="rev">
          <h1 className="font-bold">STORE</h1>
          <ul>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                About us
              </a>
            </li>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Find store
              </a>
            </li>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Categories
              </a>
            </li>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Blogs
              </a>
            </li>
          </ul>
        </div>
        <div className="rev">
          <h1 className="font-bold"> INFORMATION</h1>
          <ul>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Help center
              </a>
            </li>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Money refund
              </a>
            </li>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Shipping info
              </a>
            </li>
            <li>
              <a href="#" className="text-gray-300 hover:underline">
                Refunds
              </a>
            </li>
          </ul>
        </div>
        <div className="flex flex-col gap-2 ">
          <h1 className="font-bold">SUPPORT</h1>
          <div className="flex gap-4 flex-row md:flex-col">
            <ul>
              <li>
                <a href="#" className="text-gray-300 hover:underline">
                  Help center
                </a>
              </li>
              <li>
                <a href="#" className="text-gray-300 hover:underline">
                  Documents
                </a>
              </li>
              <li>
                <a href="#" className="text-gray-300 hover:underline">
                  Account restore
                </a>
              </li>
              <li>
                <a href="#" className="text-gray-300 hover:underline">
                  My orders
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div className="flex flex-col gap-2 ">
          <h1 className="font-bold">FOLLOW US</h1>
          <div className="flex gap-4 flex-row md:flex-col">
            <Facebook size={23} />
            <Instagram size={23} />
            <TwitterX size={23} />
            <Whatsapp size={23} />
          </div>
        </div>
      </div>
      <hr className="text-gray-400" />
      <p className="flex justify-center text-sm text-gray-400 p-3 md:p-8">
        COPYRIGHT 2023 COMPANY ALL RIGHTS RESERVED
      </p>
    </footer>
  );
}
